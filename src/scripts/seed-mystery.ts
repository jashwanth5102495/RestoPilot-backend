...
const newMenuData: CategoryGroup[] = [
  {
...
async function updateMenu() {
  try {
    ...
    const deletedRecipes = await Recipe.deleteMany({ restaurantId });
    console.log(`Deleted ${deletedRecipes.deletedCount} old recipes`);

    const deletedDishes = await Dish.deleteMany({ restaurantId });
    console.log(`Deleted ${deletedDishes.deletedCount} old dishes`);

    const deletedCategories = await Category.deleteMany({ restaurantId });
