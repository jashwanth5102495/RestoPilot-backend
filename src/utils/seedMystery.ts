...
export async function runMysterySeedIfMissing() {
  try {
    ...
    const lastCategory = await Category.findOne({ restaurantId: restaurant._id }).sort({ displayOrder: -1 });
    let displayOrder = (lastCategory?.displayOrder || 0) + 1;

    for (const catData of menuData) {
      const catExists = await Category.findOne({ restaurantId: restaurant._id, name: catData.category });
      if (catExists) continue; // Skip already seeded categories
      ...
      for (const dishData of catData.dishes) {
        const dish = new Dish({
          ...
          name: dishData.name,
          description: (dishData as any).description || dishData.name,
